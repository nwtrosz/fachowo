    ...
    <section id="contact" className="py-16 md:py-24 bg-background overflow-hidden">
      <div className="container mx-auto px-4">
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20">
              ...
              {contactInfo.map((item, index) => {
                ...
                return (
                  <a
                    key={index}
                    href={item.href}
                    className="flex items-start gap-4 p-5 rounded-2xl bg-card border border-border hover:bg-card hover:shadow-xl hover:border-accent transition-all duration-500 group"
                  >
                    <div className="w-12 h-12 bg-background rounded-xl shadow-sm flex items-center justify-center flex-shrink-0 group-hover:bg-accent group-hover:text-accent-foreground transition-colors">
          ...
          {/* Contact Form */}
          <div className="bg-card p-6 md:p-10 rounded-3xl border border-border shadow-2xl shadow-primary/5 relative">
            ...
            {submitted ? (
              <div className="flex flex-col items-center justify-center h-full min-h-[400px] text-center">
                ...
                <Button 
                  ...
                  className="bg-primary text-primary-foreground font-bold px-8 py-4 rounded-xl hover:bg-accent transition-all shadow-lg"
                >
                  Wyślij kolejną
                </Button>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold text-muted-foreground uppercase tracking-widest ml-1">Imię i Nazwisko</label>
                  <input
                    type="text"
                    name="name"
                    value={formData.name}
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-muted-foreground uppercase tracking-widest ml-1">Wybierz Filię</label>
                    <select
                      name="branch"
                      value={formData.branch}
                      onChange={handleChange}
                      required
                      className="w-full px-5 py-4 bg-muted/30 border border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-accent focus:bg-card text-foreground transition-all appearance-none cursor-pointer"
                    >
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold text-muted-foreground uppercase tracking-widest ml-1">Wiadomość</label>
                  <textarea
                    name="message"
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full px-8 py-5 bg-primary text-primary-foreground font-bold rounded-xl hover:bg-accent transition-all shadow-xl shadow-primary/10 active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-3 text-lg cursor-pointer"
                >
